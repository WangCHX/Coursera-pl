...
;; Problem 1
(define (racketlist->mupllist ns)
  (if (null? ns)
      (aunit)
      (apair (car ns) (racketlist->mupllist (cdr ns)))))

;; Problem 2
(define (mupllist->racketlist ns)
  (if (aunit? ns)
      null
      (cons (apair-e1 ns) (mupllist->racketlist (apair-e2 ns)))))
...
(define (eval-under-env e env)
  (cond [(var? e)
        ...
        [(call? e) (let ([cls (eval-under-env (call-funexp e) env)]
                     ...
                     (if (closure? cls)
                         (let* ([cls-env (closure-env cls)]
                                ...
                                [bod (fun-body func)]
                                [new-env (cons arg-bind (if (fun-nameopt func)
                                                            (cons (cons (fun-nameopt func) cls) cls-env)
                                                            cls-env))])
                           (eval-under-env bod new-env))
        ...
        [(snd? e) (let ([v (eval-under-env (snd-e e) env)])
                    (if (apair? v)
                        (apair-e2 v)
                        (error "MUPL snd applied ot non apair")))]
        [(aunit? e) e]
...
(define mupl-map 
  (fun #f "map"
       (fun "new-map" "ls"
            (ifaunit (var "ls") (aunit) (apair (call (var "map") (fst (var "ls")))
                                               (call (var "new-map") (snd (var "ls"))))))))
